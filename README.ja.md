...
呼び出しごとの prefix は習慣に負ける —— `rundiff --` を覚えねばならない agent は忘れるし、
ベースラインは溜まらない。任意の `PreToolUse` hook は、包む価値のあるコマンドだけを包む。
agent は `go test ./...` と打ち、返ってくるのは差分になる。

```sh
rundiff hook print          # settings.json スニペット（コメント付き）
rundiff hook print --json   # 素のオブジェクト（jq / chezmoi でマージする用）
```

rundiff は**あなたの Claude Code 設定を書き換えない** —— `hook print` は stdout に出すだけで、
マージするのはあなた（か dotfiles）。

**書き換える対象**: 認識できるテストツールの、**メタ文字の無い直接呼び出し** ——
`go test` / `pytest` / `jest` / `vitest run` / `cargo test` / `tsc` / `eslint`（素、または
`npx` / `pnpm exec` / `python -m` 経由）。先頭に `cd <dir> && ` が1つ付く形までは許す。
**それ以外は書き換えない。** パイプ・リダイレクト・`&&` 連結・グロブ・クォート付き引数・
`$(…)`・env prefix・`--watch`/`--json`・`npm test` は意図的に放置する:

- rundiff はコマンドの stdout を**JSON レコード + 差分に置き換える**。stdout を下流が読む
  コマンドを書き換えれば壊れる。
- `npm test` はスクリプト本体が見えず、**watcher** かもしれない —— watcher を包むと
  出力ゼロのまま永久にハングする（rundiff はバッファし、TTY を与えない）。
- 素の `vitest` も watch モード。書き換えるのは `vitest run` だけ。
...
——あなたの代わりに何も承認しない。よって Claude Code は**書き換え後**の文字列をあなたの
ルールに照らす。スニペットがツールごとに狭いエントリ（`Bash(rundiff -- go test:*)` 等）を
持つのはそのため。⚠ **絶対に** それらを `Bash(rundiff:*)` にまとめないこと: rundiff は渡された
argv を何でも実行するので、その1行は仮装した `Bash(*)` に等しい。

書き換えは **best-effort**。他の `PreToolUse` hook が居ると Claude Code が取り落とすことが
あるが、何も壊れない —— コマンドが素で走り、差分の代わりに全出力が返るだけ。
